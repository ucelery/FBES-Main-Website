...
function GalleryItemRow(props) {
    ...
    const [image_url, setImage] = useState(gallery.image);

    const deleteRow = async (id) => {
        const res = await ApiController.post("https://inventory-system-server-44k0.onrender.com/api/secure/event/delete", {
            event_id: id
        });
    ...
    const saveEdits = async (id) => {
        const res = await ApiController.post("https://inventory-system-server-44k0.onrender.com/api/secure/event/update", {
            event_id: id,
            title: title,
        ...
        gallery.description = res.description;
        gallery.image_url = res.image_url;
    }
            ...
            <Col style={{ paddingLeft: "3px", paddingLeft: "3px", fontFamily: "consolas" }}>
                {
                    isEditing && !isHeader ? (
                        <FloatingLabel controlId="title" label="Title">
                            <Form.Control size="sm" type="text" placeholder="Title" defaultValue={gallery._id} disabled
            ...
            <Col onClick={() => setIsEditing(true)} style={{ paddingLeft: "3px", paddingLeft: "3px", cursor: "pointer" }}>
                {
                    isEditing && !isHeader ? (
                        <FloatingLabel controlId="title" label="Title">
                            <Form.Control size="sm" type="text" placeholder="Title" defaultValue={gallery.title}
                                onChange={(event) => setTitle(event.target.value)}
                            />
                        </FloatingLabel>
                    ) : (
            ...
            <Col onClick={() => setIsEditing(true)} style={{ paddingLeft: "3px", paddingLeft: "3px", cursor: "pointer" }}>
                {
                    isEditing && !isHeader ? (
                        <FloatingLabel controlId="description" label="Description">
                            <Form.Control size="sm" type="text" placeholder="Description" defaultValue={gallery.description}
            ...
            <Col onClick={() => setIsEditing(true)} style={{ paddingLeft: "3px", paddingLeft: "3px", cursor: "pointer" }}>
                {
                    isEditing && !isHeader ? (
                        <FloatingLabel controlId="image" label="Image">
                            <Form.Control size="sm" type="text" placeholder="Image" defaultValue={gallery.image_url}
            ...
            </Col>
            {
                isEditing && !isHeader ? (
                    <Col style={{ paddingLeft: "3px", paddingLeft: "3px" }}>
                        <Row>
                            <Col>
                                <Button variant="outline-success" onClick={() => saveEdits(gallery._id)}>
                            ...
                            <Col>
                                <Button variant="outline-warning" onClick={() => deleteRow(gallery._id)}>
